...
function Liste({categoryID}) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const productsResponse = await axios.get(`http://localhost:8081/getProducts/${categoryID}`);
                const categoryResponse = await axios.get(`http://localhost:8081/getCategoryName/${categoryID}`);
                const imagesResponse = await axios.get(`http://localhost:8081/getProductsImg/${categoryID}`);
    ...
    const findStock = async (id) => {
        try {
            const response = await axios.get(`http://localhost:8081/getStock/${id}`);
    ...
    const findImg = async (id) => {
        try {
            const response = await axios.get(`http://localhost:8081/getImage/${id}`);
            if (response.data && response.data.length > 0) {
